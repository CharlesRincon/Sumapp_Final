...
> Stack: Unity 6000.3.11f1 · Photon Fusion 2.x · C# 9.0 · .NET Standard 2.1 · Android 8.0+
...
**Manager:** *(handled inline by TurnManager / CardManager)*
...
`DecisionScore` accumulates throughout the game based on the *quality* of individual decisions:
...
| `OnBasinCollapsedEvent` | *(none)* | `BasinService` | `GameManager` → Defeat state |
...
| `OnPassiveEffectsAppliedEvent` | *(none)* | `ProjectManager` | `GameManager` (advance round) |
...
*Document version 1.0 — generated from GDD v1 — Sumak development team*
*Keep in sync with GDD updates. Living document.*
