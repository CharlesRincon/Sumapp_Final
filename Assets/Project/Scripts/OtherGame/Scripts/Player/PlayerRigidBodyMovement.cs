...
public class PlayerRigidBodyMovement : NetworkBehaviour
{
    ...
    public override void FixedUpdateNetwork()
    {
        ...
        Velocity = _rb.Rigidbody.linearVelocity;
    }
    ...
    void UpdateMovement(InputData input)
    {
        ...
        if (input.GetButton(InputButton.LEFT) && _behaviour.InputsAllowed)
        {
            ...
            if (_rb.Rigidbody.linearVelocity.x > 0 && IsGrounded)
            {
                _rb.Rigidbody.linearVelocity *= Vector2.up;
            }
        ...
        else
        {
            ...
            if (IsGrounded)
                _rb.Rigidbody.linearVelocity *= _groundHorizontalDragVector;
            else
                _rb.Rigidbody.linearVelocity *= _airHorizontalDragVector;
        }
    ...
    private void LimitSpeed()
    {
        ...
        if (Mathf.Abs(_rb.Rigidbody.linearVelocity.x) > _maxVelocity)
        {
            _rb.Rigidbody.linearVelocity *= _horizontalSpeedReduceVector;
        }

        if (Mathf.Abs(_rb.Rigidbody.linearVelocity.y) > _maxVelocity * 2)
        {
            _rb.Rigidbody.linearVelocity *= _verticalSpeedReduceVector;
        }
    }
    ...
    private void Jump(NetworkButtons pressedButtons)
    {
        ...
        if (pressedButtons.IsSet(InputButton.JUMP) || CalculateJumpBuffer())
        {
            if (_behaviour.InputsAllowed)
            {
                ...
                {
                    _rb.Rigidbody.linearVelocity *= Vector2.right; //Reset y Velocity
                ...
                {
                    _rb.Rigidbody.linearVelocity *= Vector2.zero; //Reset y and x Velocity
    ...
    private void BetterJumpLogic(InputData input)
    {
        ...
        if (_rb.Rigidbody.linearVelocity.y < 0)
        {
            if (_wallSliding && input.AxisPressed())
            {
                _rb.Rigidbody.linearVelocity += Vector2.up * Physics2D.gravity.y * (wallSlidingMultiplier - 1) * Runner.DeltaTime;
            }
            else
            {
                _rb.Rigidbody.linearVelocity += Vector2.up * Physics2D.gravity.y * (fallMultiplier - 1) * Runner.DeltaTime;
            }
        }
        else if (_rb.Rigidbody.linearVelocity.y > 0 && !input.GetButton(InputButton.JUMP))
        {
            _rb.Rigidbody.linearVelocity += Vector2.up * Physics2D.gravity.y * (lowJumpMultiplier - 1) * Runner.DeltaTime;
        }
    }
    #endregion
}
